...
matvec_sse()
{
        ...
        //? step trough 
        for (i = 0; i < SIZE; ++i) {
                const float *row_a = &mat_a[MINDEX(i, 0)];
                __m128 acc = _mm_setzero_ps();          /* running sum of 4 floats */

                /* Step through the row and the vector four floats at a time */
                for (j = 0; j < SIZE; j += 4) {
                        __m128 a = _mm_load_ps(row_a + j);   /* 4× A[i, j‥j+3]   */
                        __m128 b = _mm_load_ps(&vec_b[j]);   /* 4× B[j‥j+3]      */
                        acc = _mm_add_ps(acc, _mm_mul_ps(a, b));
                }

                /* Horizontal add:  [x0 x1 x2 x3] → x0+x1+x2+x3 in lane 0 */
                acc = _mm_hadd_ps(acc, acc); /* x0+x1 , x2+x3 , … , … */
                acc = _mm_hadd_ps(acc, acc); /* (x0+x1)+(x2+x3) , …   */

                vec_c[i] = _mm_cvtss_f32(acc);           /* store final scalar */
        }
}
